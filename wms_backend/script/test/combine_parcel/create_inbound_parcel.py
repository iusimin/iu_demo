...
from wms_backend.lib.combine_parcel.data_accessor.inbound_parcel_accessor import \
    CPInboundParcelAccessor
from wms_backend.lib.combine_parcel.utilities.inbound_parcel_util import \
    InboundParcelUtil
from wms_backend.model.mongo.combine_parcel.inbound_parcel import \
    CPInboundParcel
from wms_backend.model.mongo.warehouse_info import CPWarehouse
from wms_backend.server import ConfigParser, IUWMSBackendService
...
def inbound_parcel():
    inbound_parcels = CPInboundParcel.objects.filter(
        {
            "created_datetime": {
                "$gte": datetime(2019, 4, 10, 0, 0, 0),
                "$lte": datetime(2019, 4, 11, 0, 0, 0)
            }
        }
    )
